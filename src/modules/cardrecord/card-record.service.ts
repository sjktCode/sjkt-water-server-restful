...
export class CardRecordService {
    ...
    async addCardForStudent(studentId: string, cardIds: string[]): Promise<boolean> {
        ...
        for (let i = 0; i < cardIds.length; i++) {
            ...
            cardRecord.organization = card.organization;
            // 创建存储的实例
            const cr = this.cardRecordRepository.create(cardRecord);
            crs.push(cr);
        }
    ...
    async create(entity: DeepPartial<CardRecord>): Promise<boolean> {
        const res = await this.cardRecordRepository.save(this.cardRecordRepository.create(entity));
        if (res) {
            return true;
        }
        return false;
    ...
    async findValidCards(studentId: string): Promise<CardRecord[]> {
        const options: FindManyOptions<CardRecord> = {
            ...
            relations: ['card', 'course', 'course.organization', 'course.teachers'],
            order: {
                createdAt: 'DESC',
            },
        };
        ...
        res.forEach((item) => {
            ...
            if (dayjs().isBefore(item.endTime)) {
                ...
                if (!((item.card.type as CardType) === CardType.TIME && item.residueTime === 0)) {
                    data.push(item);
                }
            }
        });
    ...
    async findUseCards(studentId: string, courseId: string): Promise<[CardRecord[], number]> {
        ...
        cards.forEach((card) => {
            if (!dayjs().isAfter(card.endTime)) {
                // 没有过期
                if ((card.card.type as CardType) === CardType.DURATION) {
                    // 没有过期，且是时长卡
                    newCards.push(card);
                }
                if ((card.card.type as CardType) === CardType.TIME && card.residueTime > 0) {
                    // 没有过期，且是次卡，同时还有次数
                    newCards.push(card);
                }
            }
