...
export class ScheduleRecordResolver {
    ...
    async getScheduleRecordInfo(@Args('id') id: string): Promise<ScheduleRecordResult> {
    ...
    async getScheduleRecords(
        ...
        const [scheduleRecords, total] = await this.scheduleRecordService.findScheduleRecords({
    ...
    async cancelSubscribeCourse(
        ...
        // 已经取消的状态
        if ((scheduleRecord.status as ScheduleStatus) === ScheduleStatus.CANCEL) {
            return {
        ...
        // 第三步，卡归还
        const cardRecord = await this.cardRecordService.findById(scheduleRecord.cardRecord.id);
        if ((cardRecord.card.type as CardType) === CardType.TIME) {
